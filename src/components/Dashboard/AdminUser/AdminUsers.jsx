...
const AdminUsers = () => {
  ...
  const handleUpdateRole = async (userId, newRole) => {
    ...
    try {
      const response = await api.put(`/usertype/${userId}/${newRole}`);
  ...
  const handleBanUser = async (userId, isBanned) => {
    try {
      const response = await api.put(`/status/${userId}/${isBanned}`);
      if (!response.data) {
        console.error(
              ...
              <div className={styles.userDetails}>
                ...
                <button
                  ...
                  onClick={() =>
                    handleUpdateRole(
                      user.id_user,
                      user.type_user === "admin" ? "user" : "admin"
                    )
                  }
                >
                ...
                <button
                  className={`${styles.bannedButton} ${
                    user.status ? styles.unbanned : styles.banned
                  }`}
